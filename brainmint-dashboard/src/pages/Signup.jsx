...
import { Loader2, UserPlus, ArrowRight, Check, Zap, Shield, Users, ChevronDown, Globe, Lock, Star } from "lucide-react";
...
export default function Signup({ onSignup }) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (loginResponse.ok && loginData.user) {
          const user = { id: loginData.user[0], name: loginData.user[1], email };
          setSuccess(true);
          setTimeout(() => onSignup(user), 900);
        }
      } else {
  ...
  const stats = [
    { icon: Users, label: "Active users", value: 24000, suffix: "+" },
    { icon: Zap,   label: "Tasks created", value: 1200000, suffix: "+" },
    { icon: Shield,label: "Uptime", value: 99, suffix: ".9%" },
  ];

  const progressPct = [name.length > 0, email.includes("@"), password.length >= 6].filter(Boolean).length / 3 * 100;
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nav-divider { width: 1px; height: 18px; background: #e2e8f0; margin: 0 8px; }
        ...
        .nav-btn-primary {
          ...
          border: none; padding: 7px 18px; border-radius: 9px; cursor: pointer;
          display: flex; align-items: center; gap: 6px;
        ...
        .su-brand.in { opacity: 1; transform: translateY(0); }
        .brand-icon {
        ...
        .su-hero { position: relative; margin-top: auto; margin-bottom: 34px; }
        ...
        .h1-line:nth-child(3) .h1-line-inner { transition-delay: 0.16s; }
        ...
        /* Stats */
        .stats-row {
          position: relative; display: flex; gap: 14px; margin-bottom: 28px;
          opacity: 0; transform: translateY(14px);
        ...
        .stat-value {
          ...
          letter-spacing: -0.03em; font-family: 'Geist Mono', monospace;
        ...
        .su-testi.in { opacity: 1; transform: translateY(0); }
        .su-testi::before {
          content: '';
        ...
        .t-star { color: #fbbf24; font-size: 13px; }
        .t-quote { font-size: 13px; line-height: 1.65; color: rgba(240,238,255,0.72); margin-bottom: 14px; font-style: italic; }
        ...
        .su-card.in { opacity: 1; transform: translateX(0); }
        ...
        .card-sub a { color: var(--accent); font-weight: 700; text-decoration: none; }
        ...
        .sso-strip {
          ...
          transition: opacity 0.5s 0.18s var(--ease), transform 0.5s 0.18s var(--ease);
        }
        ...
        .divider {
          ...
          opacity: 0; transition: opacity 0.5s 0.22s;
        }
        .divider.in { opacity: 1; }
        .div-line { flex: 1; height: 1px; background: #f1f5f9; }
        .div-txt { font-size: 11px; color: #cbd5e1; font-weight: 600; letter-spacing: 0.05em; white-space: nowrap; }
        ...
        .field:nth-child(1) { transition-delay: 0.05s; }
        .field:nth-child(2) { transition-delay: 0.12s; }
        .field:nth-child(3) { transition-delay: 0.19s; }

        .f-label { font-size: 12px; font-weight: 600; color: #374151; display: flex; align-items: center; justify-content: space-between; }
        .f-hint { font-size: 11px; font-weight: 500; transition: color 0.2s; }
        ...
        .su-btn:not(:disabled):hover { transform: translateY(-2px); box-shadow: 0 14px 36px rgba(124,92,252,0.32); }
        .su-btn:not(:disabled):hover::before { opacity: 1; }
        .su-btn:not(:disabled):active { transform: translateY(0); }
        .su-btn:disabled { background: #94a3b8; cursor: not-allowed; }
        .su-btn:disabled::before { opacity: 0; }
        .btn-inner { position: relative; display: flex; align-items: center; gap: 8px; }
        .btn-arr { transition: transform 0.25s var(--ease); }
        .su-btn:not(:disabled):hover .btn-arr { transform: translateX(4px); }

        /* Perks */
        .perks {
          display: flex; gap: 10px; margin-top: 14px;
          opacity: 0; transform: translateY(8px);
          transition: opacity 0.5s 0.38s var(--ease), transform 0.5s 0.38s var(--ease);
        }
        .perks.in { opacity: 1; transform: translateY(0); }
        .perk { flex: 1; display: flex; align-items: center; gap: 5px; font-size: 11px; color: #64748b; font-weight: 500; }
        .perk-dot {
        ...
        .trust-item { display: flex; align-items: center; gap: 5px; font-size: 10.5px; font-weight: 500; color: #94a3b8; }
        ...
        .terms.in { opacity: 1; }
        .terms a { color: #64748b; text-decoration: underline; }
        ...
        .footer-brand-name { font-size: 17px; font-weight: 800; color: #f0eeff; letter-spacing: -0.03em; }
        .footer-brand-desc { font-size: 13px; line-height: 1.7; color: #6b6b8a; max-width: 220px; margin-bottom: 20px; }
        ...
        .footer-col-title {
          ...
          text-transform: uppercase; color: #f0eeff; margin-bottom: 16px;
        }
        ...
        .newsletter-title { font-size: 15px; font-weight: 700; color: #f0eeff; margin-bottom: 4px; letter-spacing: -0.02em; }
        .newsletter-sub { font-size: 12.5px; color: #6b6b8a; }
        .newsletter-form { display: flex; gap: 8px; flex-shrink: 0; }
        .newsletter-input {
          padding: 10px 16px;
          background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1);
          border-radius: 9px; font-size: 13px; font-family: 'Bricolage Grotesque', sans-serif;
          color: #f0eeff; outline: none; width: 230px;
          transition: border-color 0.2s, background 0.2s;
        }
        .newsletter-input::placeholder { color: #6b6b8a; }
        .newsletter-input:focus { border-color: rgba(124,92,252,0.5); background: rgba(124,92,252,0.07); }
        .newsletter-btn {
          padding: 10px 18px;
          background: linear-gradient(135deg, #7c5cfc, #a855f7);
          border: none; border-radius: 9px; font-size: 13px; font-weight: 700;
          font-family: 'Bricolage Grotesque', sans-serif; color: white; cursor: pointer;
          white-space: nowrap; transition: transform 0.15s, box-shadow 0.15s;
        }
        .newsletter-btn:hover { transform: translateY(-1px); box-shadow: 0 4px 14px rgba(124,92,252,0.4); }

        .footer-bottom {
          padding: 20px 0;
          display: flex; align-items: center; justify-content: space-between; gap: 24px; flex-wrap: wrap;
        }
        .footer-bottom-left { display: flex; align-items: center; gap: 6px; flex-wrap: wrap; }
        .footer-copy { font-size: 12px; color: #4b4b6b; }
        .footer-sep { color: #2a2a40; font-size: 11px; }
        .footer-legal-link {
          font-size: 12px; color: #4b4b6b; background: none; border: none; cursor: pointer;
          font-family: 'Bricolage Grotesque', sans-serif; transition: color 0.15s; padding: 0;
        }
        .footer-legal-link:hover { color: #6b6b8a; }
        .footer-bottom-right { display: flex; align-items: center; gap: 16px; }
        .footer-trust-chips { display: flex; gap: 6px; }
        .footer-chip {
          display: flex; align-items: center; gap: 4px;
          padding: 3px 9px; border-radius: 20px; font-size: 10.5px; font-weight: 600;
          border: 1px solid rgba(255,255,255,0.07); color: #6b6b8a; background: rgba(255,255,255,0.03);
        }
        .footer-locale {
          display: flex; align-items: center; gap: 5px;
          font-size: 12px; font-weight: 500; color: #4b4b6b;
          background: none; border: none; cursor: pointer;
          font-family: 'Bricolage Grotesque', sans-serif; transition: color 0.15s;
        }
        .footer-locale:hover { color: #6b6b8a; }

        /* Responsive Tablet Breakpoint */
        @media (max-width: 860px) {
          .su-root { grid-template-columns: 1fr; padding-top: 60px; }
          .su-left { display: none; }
          .su-right { padding: 40px 24px 32px; }
        ...
        @media (max-width: 580px) {
          ...
          .nav-btn-secondary { display: none; } /* hide secondary sign in link to save space */
          .nav-btn-primary { padding: 7px 12px; font-size: 12.5px; }
          .nav-logo-text { font-size: 15px; }

          .su-right { padding: 30px 16px 24px; }
          .card-h2 { font-size: 24px; }
          
          /* Stack the SSO buttons cleanly */
          .sso-strip { flex-direction: column; gap: 10px; }
          
          /* Wrap the inline perks/trust components */
          .perks { flex-wrap: wrap; justify-content: center; gap: 8px; }
      ...
      <nav className="top-navbar">
        <div className="nav-left">
          ...
          <div className="nav-links">
            <button className="nav-link">Features <ChevronDown size={12} /></button>
      ...
      <div className="su-root">
        ...
        <div className="su-left">
          <ParticleField />
          <Orb style={{ width:400, height:400, background:"rgba(124,92,252,0.16)", top:-90, left:-90, animationDuration:"9s" }} />
          <Orb style={{ width:280, height:280, background:"rgba(192,132,252,0.12)", bottom:50, right:-60, animationDuration:"11s", animationDelay:"2s" }} />
          <Orb style={{ width:180, height:180, background:"rgba(56,189,248,0.08)", top:"42%", right:30, animationDuration:"7s", animationDelay:"1s" }} />

          <div className={`su-brand ${mounted ? "in" : ""}`}>
          ...
          <div className="su-hero">
            <div className={`hero-pill ${mounted ? "in" : ""}`}>
              ...
              Trusted by 24,000+ teams
            </div>
            <h1 className="hero-h1">
              {["Organize your", "work with", <><span className="grad-word">precision.</span></>].map((line, i) => (
                <span className="h1-line" key={i}>
                  <span className={`h1-line-inner ${mounted ? "in" : ""}`} style={{ transitionDelay: `${0.18 + i * 0.09}s` }}>
                    {line}
                  </span>
                </span>
              ))}
            ...
            <p className={`hero-sub ${mounted ? "in" : ""}`}>
              A task management platform engineered for high-performance teams. Ship faster, stay aligned, never drop the ball.
            </p>
          </div>
          ...
          <div className={`stats-row ${mounted ? "in" : ""}`}>
            {stats.map(({ icon: Icon, label, value, suffix }) => (
              <div className="stat-card" key={label}>
                <div className="stat-icon"><Icon size={13} color="#7c5cfc" /></div>
                <div className="stat-value">{mounted ? <Counter to={value} suffix={suffix} /> : "0"}</div>
                <div className="stat-label">{label}</div>
              </div>
            ))}
          </div>

          <div className={`su-testi ${mounted ? "in" : ""}`}>
            <div className="t-stars">{[...Array(5)].map((_,i) => <span className="t-star" key={i}>★</span>)}</div>
            <p className="t-quote">"BrainMint replaced three tools for us overnight. The team was productive within the first hour of onboarding."</p>
            <div className="t-row">
              <div className="t-avatar">MK</div>
              <div>
                <div className="t-name">Marcus K.</div>
                <div className="t-role">CTO, Streamline Labs</div>
              </div>
              <div className="t-badge"><Check size={9} /> Verified</div>
            </div>
          </div>
        </div>
        ...
        <div className="su-right">
          <div className={`success-overlay ${success ? "show" : ""}`}>
            ...
            <div className="s-sub">Redirecting to your dashboard…</div>
          </div>

          <div className={`su-card ${mounted ? "in" : ""}`}>
            <div className="card-top">
              <div className="card-eyebrow">
                <div className="eyebrow-bar" />
                Free forever · No credit card
              </div>
              <h2 className="card-h2">Create your account</h2>
              <div className="card-sub">
                Already on BrainMint?
            ...
            <div className="prog-bar-track">
              <div className="prog-bar-fill" style={{ width: `${progressPct}%` }} />
            </div>
            ...
            <form onSubmit={handleSubmit} className="su-form">
              <div className={`field ${mounted ? "in" : ""}`}>
                ...
                <div className="f-wrap">
                  ...
                  <input type="text" value={name} onChange={e=>setName(e.target.value)}
                    placeholder="Jane Smith" className="f-input" required disabled={isLoading} />
                  <div className={`f-check ${name.length > 1 ? "show" : ""}`}>
              ...
              <div className={`field ${mounted ? "in" : ""}`}>
                ...
                <div className="f-wrap">
                  ...
                  <input type="email" value={email} onChange={e=>setEmail(e.target.value)}
                    placeholder="you@company.com" className="f-input" required disabled={isLoading} />
                  <div className={`f-check ${email.includes("@") && email.includes(".") ? "show" : ""}`}>
                    <Check size={9} color="white" strokeWidth={3} />
                  </div>
                </div>
              </div>
              ...
              <div className={`field ${mounted ? "in" : ""}`}>
                <label className="f-label">
                  Password
                  <span className="f-hint" style={{ color: password.length >= 6 ? "var(--success)" : "#94a3b8" }}>
                    {password.length > 0 ? (password.length >= 6 ? "✓ Good strength" : `${6 - password.length} more to go`) : "Min. 6 characters"}
                  </span>
                </label>
                <div className="f-wrap">
                  <span className="f-icon">
                  ...
                  <input type="password" value={password} onChange={e=>setPassword(e.target.value)}
                    placeholder="••••••••" className="f-input" required minLength={6} disabled={isLoading} />
                  <div className={`f-check ${password.length >= 6 ? "show" : ""}`}>
                    <Check size={9} color="white" strokeWidth={3} />
                  </div>
                </div>
              </div>
                ...
                <button type="submit" disabled={isLoading} className="su-btn">
                  <span className="btn-inner">
                    {isLoading ? (
                      ...
                      <><UserPlus size={15} /> Create Free Account <ArrowRight size={14} className="btn-arr" /></>
                    )}
                  </span>
                </button>
              </div>
            ...
            <div className={`perks ${mounted ? "in" : ""}`}>
              {["Free forever", "No setup needed", "Cancel anytime"].map(p => (
                <div className="perk" key={p}>
                  <div className="perk-dot"><Check size={9} color="#7c5cfc" strokeWidth={3} /></div>
                  {p}
                </div>
              ))}
            </div>
            ...
            <div className={`trust-row ${mounted ? "in" : ""}`}>
              <div className="trust-item"><Lock size={11} /> 256-bit SSL</div>
              <div className="trust-sep" />
              <div className="trust-item"><Shield size={11} /> SOC 2 Certified</div>
              <div className="trust-sep" />
              <div className="trust-item"><Star size={11} /> 4.9 / 5 rating</div>
            </div>

            <p className={`terms ${mounted ? "in" : ""}`}>
      ...
      <footer className="site-footer">
        <div className="footer-top">
          ...
          <div>
            ...
            <p className="footer-brand-desc">The modern project management platform built for high-velocity teams.</p>
          ...
          <div>
            ...
            <div className="footer-col-links">
              ...
              <button className="footer-col-link">Integrations <span className="footer-link-badge badge-new">New</span></button>
              <button className="footer-col-link">Pricing</button>
              <button className="footer-col-link">Changelog</button>
              <button className="footer-col-link">AI Assist <span className="footer-link-badge badge-hot">Hot</span></button>
        ...
        <div className="footer-bottom">
          ...
          <div className="footer-bottom-right">
            ...
            <button className="footer-locale">
              <Globe size={11} /> English (US) <ChevronDown size={10} />
            </button>
          </div>
        </div>
      </footer>
